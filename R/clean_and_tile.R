...
#'
#' @examples
clean_and_tile <- function(input, output_dir, verbose = TRUE,
                           chunk_size = 200, chunk_buffer = 20,
   ...
   if (n_output_tiles > 0) {
      ...
      if (skip_if_output_exists) {
         message("clean_and_tile() output already exists. Skipping dir: ",
                 output_dir)
         return(readLAScatalog(output_dir))
      }

      stop(n_output_tiles, " cleaned tiles already exist in: ", output_dir,
           " Delete to rerun.")
   }
   ...
   opt_output_files(ctg) <- file.path(output_dir,
                                      "{ID}_{XLEFT}_{YBOTTOM}_clean")
