...
#' @export
process_calibrated_folder_water_logger_elevation <- function(folder_path, deployment_file, start_date, end_date,
   ...
   for (i in seq_along(files)) {
      ...
      result <- tryCatch({
         tide_data <- calculate_water_surface_elevation(file, start_date, end_date, quantile, min_depth)
   ...
   all_results <- all_results[, -c(1, 2)]  # drops first and second columns
